...
6. **Phase 4 結構自檢**：回讀實際狀態，確認兩個匯出皆存在且指向本次推導出的儲存體、每個匯出受控識別皆具寫入權、Event Grid 訂閱指向所帶入的觸發 URL。見[失敗即中止](#失敗即中止)。
...
**新世代** Cost Management 匯出寫入儲存體時，用的**不是** LumiTure 的服務主體，而是**匯出自己的識別**。Azure 僅在目的儲存體**停用共用金鑰（shared key）存取**時，才會為匯出掛上 system-assigned 受控識別（CSP 與企業資安政策下相當常見）；該識別需具備儲存體的 `Storage Blob Data Contributor`，否則每次匯出執行皆會以 `AccessToStorageAccountDenied` 失敗。

Azure 對此**完全不會示警**：匯出存在、於 Portal 顯示正常，只是不產生任何檔案。

腳本於建立匯出後會回讀其識別，只要存在即自動授予該角色（冪等）。這一步之所以可行，是因為腳本以**客戶自身（訂用帳戶 Owner）**身分執行——LumiTure 的唯讀 SP 無權授予角色，**事後無法由 LumiTure 端補救**。若儲存體允許共用金鑰，匯出不會有受控識別，此步驟正確地不做任何事。
...
> **同名但位於不同儲存體的第二個匯出**即「世代雙胞胎」（generation twin），為早期執行殘留的舊世代匯出。Phase 4 會提出警告，請至 Portal 刪除：由 CLI 以名稱刪除會刪到錯的那一個。
