...
6. **（預設）** 回補 **3 個月歷史資料**：以一次性匯出建立，**每月一個**，讓首次進入儀表板即可看到趨勢而非僅有當月（`--backfill-months <n>` 調整、`--backfill-months 0` 略過）。僅在步驟 5 的 Event Grid 訂閱**確實建立後**才執行——一次性匯出不會重新投遞，沒有接收端時歷史資料會遺失；若步驟 5 被略過，回補也會一併略過（顯示警告），之後帶入 `--event-trigger-url` 重跑即可補齊。
