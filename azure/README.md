...
6. **(default)** Seeds **3 months of history** as one-time exports, one per month, so your first dashboard shows a trend instead of only the current month — `--backfill-months <n>` to tune, `--backfill-months 0` to skip
